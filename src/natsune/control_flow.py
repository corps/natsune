...
from .invocations import ExpansionWithAdapters, expansion_invocation, filter_invocation, send_parameter, unpack_wires, \
    merge_invocation, send_parameters, unpack_port_and_wires
...
class Loop(ExpansionWithAdapters):
    ...
    def __call__(
        ...
        with (
            ...
            iter_input_1, iter_input_2 = this_invocation.inputs.i_value.readout(True).duplicate()
            send_value(this_invocation.inputs.i_variables.readout(True), iterable_invocation.inputs.i_variables.readin())
            send_value(iter_input_1, iterable_invocation.inputs.i_value.readin())
            send_value(iter_input_2, recurse.inputs.i_value.readin())

            cond_invocation = IfThenElse(self.body.variables.adapter).invocation(executor)
            send_value(iterable_invocation.control.o_return.readout(True), cond_invocation.i_value.readin())
            send_value(iterable_invocation.control.o_finish.readout(True), cond_invocation.i_context.readin())

            send_value(cond_invocation.o_orelse.readout(True), orelse_invocation.inputs.i_variables.readin())
            send_value(cond_invocation.o_body.readout(True), body_invocation.inputs.i_variables.readin())

            orelse_invocation.control.send_to(orelse_result, True)

            # Option to return from parent
            o_return_1, o_return_2 = OneOf.share(body_result.o_return.readin(), executor)
            ...
            o_finished_1, o_finished_2 = OneOf.share(body_result.o_finish.readin(), executor)
            ...
            o_continue_1, o_continue_2 = OneOf.share(recurse.inputs.i_variables.readin(), executor)
...
class ConcurrentMerge(ExpansionWithAdapters):
    ...
    def __call__(self, exec: Executor, port: Port, wires: Sequence[Wire]) -> None:
        ...
        result1, result2 = OneOf.share(as_to_register(WirePort([wires[0]]), self.output_adapter, exec), exec)
        ...
        send_value(send_parameter(
            filter_invocation(self.should_short, exec),
            first_value_1
...
class ConcurrentMergeInvocation:
    ...
    @classmethod
    def split(cls, inputs: InterfaceRegister, outputs: InterfaceRegister) -> ConcurrentMergeInvocation:
...
class VariablesFlow(ExpansionWithAdapters):
    ...
    def __post_init__(self) -> None:
        for name, variable_input in zip(self.variables.keys(), self.flow_input.i_variables.split(True)):
...
class FlowInvocation:
    ...
    @classmethod
    def split(cls, inputs: InterfaceRegister, outputs: InterfaceRegister) -> FlowInvocation:
...
class FlowControl:
    ...
    def share(self) -> Generator[tuple[FlowControl, FlowControl]]:
        a, b = tuple(FlowControl(*parts) for parts in zip(*(OneOf.share(i.readin(), i.connector) for i in self)))
        try:
            yield a, b
        finally:
            a.close()
